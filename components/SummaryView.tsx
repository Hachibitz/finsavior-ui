...
const SummaryView: React.FC<SummaryViewProps> = ({ 
  ...
  const getLastSixMonths = useCallback((referenceMonth: string) => {
    const months = [];
  ...
  useEffect(() => {
    ...
    fetchHistory();
  }, [selectedMonth, getLastSixMonths]);
  ...
  const savingsLabel = savingsRate > 30 ? t('dashboard.excellent') : savingsRate > 10 ? t('dashboard.good') : t('dashboard.attention');

  const currencyMask = useMemo(() => {
    const symbol = formatCurrency(0).replace(/[\d.,\s\-0]/g, '').trim() || 'R$';
    return `${symbol} ••••••••`;
  }, [i18n.language]);
  ...
  const formatTransactionDate = (purchaseDate?: string, fallbackDate?: string) => {
    const source = purchaseDate || fallbackDate;
    if (!source) return '';
    const iso = source.match(/^(\d{4})-(\d{2})-(\d{2})/);
    if (iso) {
      return formatShortDate(new Date(Number(iso[1]), Number(iso[2]) - 1, Number(iso[3])));
    }
  ...
  const handleConfirmAnalysis = async (data: AiAdviceDTO) => {
    ...
    try {
      ...
      onAddNotification?.({
        title: t('dashboard.reportReadyTitle'),
        message: t('dashboard.reportReadyMsg'),
        type: 'success',
        actionUrl: 'ai',
      ...
      onAddNotification?.({
        title: t('dashboard.reportErrorTitle'),
        message: translateApiError(error, t('dashboard.reportErrorMsg')),
        type: 'error'
      });
    } finally {
    ...
    <div className="space-y-6 animate-slide-up pb-10">
      ...
      {(!profile?.plan || profile.plan.planId === 'FREE' || profile.plan.planDs === 'FREE') && !isBannerClosed && (
        <div className="bg-gradient-to-r from-primary/20 via-accent/20 to-primary/20 border border-primary/30 p-6 rounded-[2rem] flex flex-col md:flex-row items-center justify-between gap-6 relative overflow-hidden group">
          ...
          <div className="flex items-center gap-5 relative z-10">
            ...
            <div>
              <h3 className="text-xl font-black text-white tracking-tight">{t('dashboard.premiumBannerTitle')}</h3>
              <p className="text-slate-400 text-sm">{t('dashboard.premiumBannerDesc')}</p>
            </div>
          </div>
          ...
          <div className="flex-1">
            <div className="flex items-center gap-2 text-indigo-300/80 mb-2">
              ...
              <div className="flex items-center gap-1.5">
                <span className="text-[10px] md:text-xs font-black uppercase tracking-widest">{t('dashboard.netWorth')}</span>
              ...
              <button 
                ...
                title={isBalanceVisible ? t('dashboard.hideBalance') : t('dashboard.showBalance')}
              >
            ...
            <div className="flex flex-col md:flex-row md:items-center gap-4 md:gap-8">
              ...
              <div className="flex items-center gap-3">
                <div className="flex items-center gap-1.5 bg-emerald-500/10 border border-emerald-500/20 px-3 py-1.5 rounded-xl text-emerald-400 font-black text-[10px] uppercase tracking-wider">
                  <TrendingUp size={12} />
                  <span>{isBalanceVisible ? `+${formatCurrency(summary.totalIncome)}` : currencyMask}</span>
                </div>
                <div className="flex items-center gap-1.5 bg-rose-500/10 border border-rose-500/20 px-3 py-1.5 rounded-xl text-rose-400 font-black text-[10px] uppercase tracking-wider">
                  ...
                  <span>{isBalanceVisible ? `-${formatCurrency(summary.totalExpense)}` : currencyMask}</span>
                </div>
              </div>
            </div>
          </div>
          ...
          <div className="flex flex-row md:flex-col items-center md:items-end justify-between md:justify-center gap-4 pt-4 md:pt-0 border-t md:border-t-0 border-white/5">
             <div className="text-left md:text-right">
                <div className="flex items-center md:justify-end gap-1.5 mb-1">
                  <p className="text-slate-500 text-[10px] font-black uppercase tracking-widest">{t('dashboard.savingsRate')}</p>
                  <div className="relative group">
                    ...
                    <div className="absolute bottom-full right-0 mb-2 w-48 p-3 bg-slate-800 border border-white/10 rounded-xl shadow-2xl opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all z-[60] pointer-events-none">
                      <p className="text-[10px] text-slate-300 leading-relaxed normal-case font-medium">
                        {t('dashboard.savingsRateTooltip')}
                      </p>
                    </div>
                  </div>
                </div>
      ...
      {showScrollHint && (
        <div className="md:hidden flex flex-col items-center gap-1 py-4 animate-bounce opacity-50">
          <p className="text-[10px] font-bold text-slate-500 uppercase tracking-widest">{t('dashboard.scrollHint')}</p>
      ...
      >
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-2 mb-0.5">
             <span className="text-[10px] font-black text-primary uppercase tracking-widest">{t('dashboard.saviInsight')}</span>
          ...
          <p className="text-slate-300 text-sm font-medium truncate italic">
            {loadingTip ? t('dashboard.analyzingPatterns') : aiTip || t('dashboard.keepFocus')}
          </p>
        </div>
        <button 
          onClick={(e) => {
          ...
          <div className="bg-slate-900 w-full max-w-lg rounded-[2.5rem] border border-white/10 shadow-2xl overflow-hidden animate-slide-up" onClick={e => e.stopPropagation()}>
            <div className="p-8 border-b border-white/5 flex justify-between items-center bg-gradient-to-r from-primary/10 to-transparent">
              <div className="flex items-center gap-3">
                <SaviIcon className="w-16 h-16" />
                <div>
                  <h3 className="text-xl font-black text-white tracking-tight">{t('dashboard.saviInsight')}</h3>
                  <p className="text-[10px] font-bold text-slate-500 uppercase tracking-widest">{t('dashboard.intelligenceAnalysis')}</p>
                </div>
              </div>
            ...
            <div className="p-8">
              ...
              <div className="mt-8 pt-8 border-t border-white/5 flex justify-between items-center">
                ...
                >
                  <RotateCcw size={14} />
                  {t('dashboard.regenerateAnalysis')}
                </button>
              </div>
            </div>
          </div>
          ...
          <div className="glass-card p-6 rounded-[2rem] border border-white/5">
            <div className="flex justify-between items-center mb-8">
              <div className="flex items-center gap-3">
                ...
                <h3 className="font-bold text-white">{t('dashboard.monthlyEvolution')}</h3>
              </div>
              <div className="flex gap-2">
                <div className="flex items-center gap-4 px-3 py-1 bg-white/5 rounded-xl">
                   <div className="flex items-center gap-1.5">
                      <div className="w-2 h-2 rounded-full bg-emerald-500" />
                      <span className="text-[10px] font-bold text-slate-400 uppercase tracking-tighter">{t('dashboard.income')}</span>
                   </div>
                   <div className="flex items-center gap-1.5">
                      <div className="w-2 h-2 rounded-full bg-rose-500" />
                      <span className="text-[10px] font-bold text-slate-400 uppercase tracking-tighter">{t('dashboard.expenses')}</span>
                   </div>
                </div>
              </div>
            </div>
            ...
            <div className="h-[300px] w-full relative">
              {isLoadingHistory ? (
                <div className="absolute inset-0 flex flex-col items-center justify-center gap-3 bg-slate-900/20 backdrop-blur-sm rounded-2xl z-10">
                  ...
                  <p className="text-xs font-bold text-slate-500 uppercase tracking-widest">{t('dashboard.calculatingTrends')}</p>
                </div>
              ) : null}
            ...
            <div className="mt-6 flex flex-wrap justify-center gap-6">
               <div className="flex items-center gap-2">
                  ...
                  <span className="text-[11px] font-bold text-slate-400 uppercase tracking-wider">{t('dashboard.income')}</span>
               </div>
               <div className="flex items-center gap-2">
                  <div className="w-3 h-3 rounded-full border-2 border-rose-500 bg-rose-500/20" />
                  <span className="text-[11px] font-bold text-slate-400 uppercase tracking-wider">{t('dashboard.expenses')}</span>
               </div>
               <div className="flex items-center gap-2">
                  <div className="w-3 h-3 rounded-full border-2 border-blue-500 bg-blue-500/20" />
                  <span className="text-[11px] font-bold text-slate-400 uppercase tracking-wider">{t('dashboard.savings')}</span>
               </div>
               <div className="flex items-center gap-2">
                  <div className="w-3 h-3 rounded-full border-2 border-amber-500 bg-amber-500/20" />
                  <span className="text-[11px] font-bold text-slate-400 uppercase tracking-wider">{t('dashboard.cardTotal')}</span>
               </div>
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
             <div className="glass-card p-6 rounded-[2rem] border border-white/5">
                <div className="flex items-center gap-3 mb-6">
                  ...
                  <h3 className="font-bold text-white">{t('dashboard.spendingByCategory')}</h3>
                </div>
             ...
             <div className="glass-card p-6 rounded-[2rem] border border-white/5 flex flex-col justify-between">
                <div>
                  <div className="flex items-center gap-3 mb-6">
                    ...
                    <h3 className="font-bold text-white">{t('dashboard.closingForecast')}</h3>
                  </div>
                  ...
                  <p className="text-xs text-slate-500 mt-2">{t('dashboard.forecastDesc')}</p>
                </div>
                
                <div className="mt-6 p-4 bg-white/5 rounded-2xl border border-white/5">
                   <div className="flex justify-between items-center mb-2">
                      <span className="text-[10px] font-black text-slate-500 uppercase">{t('dashboard.financialHealth')}</span>
        ...
        <div className="space-y-6">
           <div className="glass-card p-6 rounded-[2rem] border border-white/5 h-full">
              <div className="flex items-center justify-between mb-8">
                <h3 className="font-bold text-white">{t('dashboard.recentActivity')}</h3>
                <button 
              ...
              <div className="space-y-6">
                {recentTransactions.map((tx, idx) => {
                  const category = categories.find(c => c.id === (tx as any).category);
                    ...
                    <div key={idx} className="flex items-center justify-between group">
                      <div className="flex items-center gap-4">
                        ...
                        <div className="min-w-0">
                          <p className="font-bold text-white text-sm truncate w-32 md:w-40">{tx.description}</p>
                          <p className="text-[10px] text-slate-500 font-medium uppercase tracking-wider">
                            {formatTransactionDate((tx as any).purchaseDate, tx.date)}
                          </p>
                        </div>
                      </div>
                      <div className="text-right">
                        ...
                        <p className="text-[9px] text-slate-600 font-bold uppercase tracking-tighter">
                          {isIncome ? t('dashboard.deposit') : (tx as any).cardId ? t('dashboard.card') : t('dashboard.debit')}
                        </p>
                      </div>
                    </div>
                  );
                ...
                {recentTransactions.length === 0 && (
                  <div className="text-center py-20">
                     ...
                     <p className="text-slate-500 text-sm">{t('dashboard.noRecentActivity')}</p>
                  </div>
                )}
              </div>
              ...
              >
                 ...
                 {isGeneratingReport ? t('dashboard.generatingReport') : t('dashboard.generateFullReport')}
              </button>
           </div>
        </div>
